...
@Component
@Profile("mysqldb")
public class MysqlDBInit extends BaseDBInit {
    private final static Logger logger = LoggerFactory.getLogger(MysqlDBInit.class);

    private EventSqlSchema eventSqlSchema;
    private SagaSqlSchema sagaSqlSchema;
    private DataSource dataSource;
    private UserQueryRepository userQueryRepository;
    private InstrumentQueryRepository companyQueryRepository;
    ...
    @Autowired
    public MysqlDBInit(CommandBus commandBus,
                      ...
                      UserQueryRepository userQueryRepository, InstrumentQueryRepository companyQueryRepository, OrderBookQueryRepository orderBookQueryRepository, PortfolioQueryRepository portfolioQueryRepository, TradeExecutedQueryRepository tradeExecutedQueryRepository, TransactionQueryRepository transactionQueryRepository) {
    ...
    @Override
    void initializeDB() {
        logger.debug("Initialize the mysqldb database.");
    ...
    void additionalDBSteps() {
        logger.debug("Additional steps for the mysqldb database.");
    }

    @Override
    public Set<String> obtainCollectionNames() {
        // TODO jettro: Implement this
        logger.debug("Obtain collections from mysqldb.");
        return null;
    }
    ...
    public PreparedStatement sql_dropSnapshotEventEntryTable(Connection connection) throws SQLException {
        return connection.prepareStatement("drop table if exists SNAPSHOTEVENTENTRY;");
    }

    public PreparedStatement sql_dropDomainEventEntryTable(Connection connection) throws SQLException {
        return connection.prepareStatement("drop table if exists DOMAINEVENTENTRY;");
    }

    public PreparedStatement sql_dropTableAssocValueEntry(Connection conn) throws SQLException {
        return conn.prepareStatement("drop table if exists ASSOCIATIONVALUEENTRY;");
    }

    public PreparedStatement sql_dropTableSagaEntry(Connection conn) throws SQLException {
        return conn.prepareStatement("drop table if exists SAGAENTRY;");
    }
